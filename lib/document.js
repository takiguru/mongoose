...
function Document(obj, fields, skipId, options) {
  ...
  if (this.$__schema == null) {
    const _schema =
      utils.isObject(fields) && !fields.instanceOfSchema ? new Schema(fields) : fields;
  ...
  if (!this.$__.strictMode && obj) {
    ...
    keys.forEach(function (key) {
      ...
      if (
        !(key in schema.tree) &&
        !(key in schema.methods) &&
        !(key in schema.virtuals) &&
        !key.startsWith('$')
      ) {
...
Document.prototype.$__init = function (doc, opts) {
  ...
  const hasIncludedChildren =
    this.$__.exclude === false && this.$__.selected
    ...
    if (!schemaType && utils.isPOJO(value)) {
      ...
      if (!doc[i]) {
        ...
        if (
          !strict &&
          !(i in docSchema.tree) &&
          !(i in docSchema.methods) &&
          !(i in docSchema.virtuals)
        ) {
          self[i] = doc[i];
        ...
        if (schemaType && !wasPopulated && !opts.hydratedPopulatedDocs) {
          ...
          if (doc[i] && doc[i].$__ && doc[i].$__.wasPopulated) {
            ...
            self.$populated(
              path,
              doc[i]
                .map((populatedDoc) => populatedDoc?.$__?.wasPopulated?.value)
                .filter((val) => val != null),
              doc[i][0].$__.wasPopulated.options,
            );
          }
        } else {
          doc[i] = value;
...
Document.prototype.$session = function $session(session) {
  ...
  if (session != null && session.hasEnded) {
    throw new MongooseError(
      "Cannot set a document's session to a session that has ended. Make sure you haven't " +
...
Document.prototype.overwrite = function overwrite(obj) {
  ...
  for (const key of keys) {
    ...
    if (
      this.$__schema.options.discriminatorKey &&
      key === this.$__schema.options.discriminatorKey
    ) {
      continue;
    }
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (path == null) {
    ...
    if (path instanceof Document) {
      if (path.$__isNested) {
        ...
        path =
          path.$__schema === this.$__schema ? applyVirtuals(path, { ...path._doc }) : path._doc;
      }
    }
    if (path == null) {
      [path, val] = [val, path];
  ...
  if (pathType === 'nested' && val) {
    if (typeof val === 'object' && val != null) {
      ...
      if (
        this.$__.savedState != null &&
        !this.$isNew &&
        !this.$__.savedState.hasOwnProperty(path)
      ) {
  ...
  if (pathType === 'adhocOrUndefined' && strict) {
    ...
    for (i = 0; i < parts.length; ++i) {
      ...
      if (schema instanceof MixedSchema) {
        ...
        schema.$isSchemaMap &&
        schema.$__schemaType instanceof MixedSchema &&
        i < parts.length - 1
      ) {
  ...
  try {
    ...
    const refMatches = (() => {
      ...
      // Check ref
      const refOpt =
        typeof schema.options.ref === 'function' && !schema.options.ref[modelSymbol]
          ? schema.options.ref.call(this, this)
          : schema.options.ref;
    ...
    if (
      ...
      (!val.$__.wasPopulated || utils.deepEqual(val.$__.wasPopulated.value, val._doc._id))
    ) {
      const unpopulatedValue =
        schema && schema.$isSingleNested ? schema.cast(val, this) : val._doc._id;
  ...
  if (shouldSet) {
    ...
    const isModifiedWithinTransaction =
      this.$__.session &&
...
Document.prototype.$__set = function (
  ...
  if (shouldModify) {
    ...
    Array.isArray(val) &&
    Array.isArray(priorVal) &&
    utils.isMongooseArray(val) &&
    utils.isMongooseArray(priorVal)
  ) {
...
Document.prototype.directModifiedPaths = function () {
  // DEBUG: Log when activePaths is problematic
  if (!this.$__) {
    console.error('[MONGOOSE DEBUG] directModifiedPaths: this.$__ is undefined/null');
    console.error('[MONGOOSE DEBUG] this._doc:', JSON.stringify(this._doc, null, 2));
    console.error('[MONGOOSE DEBUG] this.constructor.name:', this.constructor?.name);
    console.error('[MONGOOSE DEBUG] this.$isNew:', this.$isNew);
    console.error('[MONGOOSE DEBUG] Stack:', new Error().stack);
    return [];
  }
  if (!this.$__.activePaths) {
    console.error('[MONGOOSE DEBUG] directModifiedPaths: this.$__.activePaths is undefined/null');
    console.error('[MONGOOSE DEBUG] this.$__ keys:', Object.keys(this.$__));
    console.error(
      '[MONGOOSE DEBUG] this.$__:',
      JSON.stringify(
        this.$__,
        (key, value) => {
          if (key === 'parent' || key === 'ownerDocument') return '[circular]';
          return value;
        },
        2,
      ),
...
Document.prototype.modifiedPaths = function (options) {
  ...
  // DEBUG: Log when activePaths is problematic
  let directModifiedPaths;
  if (!this.$__) {
    console.error('[MONGOOSE DEBUG] modifiedPaths: this.$__ is undefined/null');
    console.error('[MONGOOSE DEBUG] this._doc:', JSON.stringify(this._doc, null, 2));
    console.error('[MONGOOSE DEBUG] this.constructor.name:', this.constructor?.name);
    console.error('[MONGOOSE DEBUG] this.$isNew:', this.$isNew);
    console.error('[MONGOOSE DEBUG] Stack:', new Error().stack);
    directModifiedPaths = [];
  } else if (!this.$__.activePaths) {
    console.error('[MONGOOSE DEBUG] modifiedPaths: this.$__.activePaths is undefined/null');
    console.error('[MONGOOSE DEBUG] this.$__ keys:', Object.keys(this.$__));
    console.error(
      '[MONGOOSE DEBUG] this.$__:',
      JSON.stringify(
    ...
    directModifiedPaths = Object.keys(this.$__.activePaths.getStatePaths('modify'));
  }
  ...
  if (paths) {
    ...
    if (ignoreAtomics) {
      directModifiedPaths = directModifiedPaths.filter((path) => {
        ...
        if (
          value != null &&
          value[arrayAtomicsSymbol] != null &&
          value[arrayAtomicsSymbol].$set === undefined
        ) {
          return false;
        }
        return true;
  ...
  if (typeof path === 'string' && path.indexOf(' ') === -1) {
    ...
    for (let i = 0; i < pieces.length - 1; ++i) {
      ...
      if (
        subdoc != null &&
        subdoc.$__ != null &&
        subdoc.isDirectModified(pieces.slice(i + 1).join('.'))
      ) {
        return true;
      }
    }
...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  if (
    typeof pathsToValidate === 'function' ||
...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  let paths = new Set(
    Object.keys(doc.$__.activePaths.getStatePaths('require')).filter(function (path) {
  ...
  if (!isNestedValidate) {
    ...
    for (const subdoc of topLevelSubdocs) {
      if (subdoc.$basePath) {
        ...
        if (subdocParent.$__ == null) {
          // Debug: log info about the problematic subdoc
          console.log('[MONGOOSE DEBUG] subdocParent.$__ is null/undefined');
          console.log('[MONGOOSE DEBUG] doc.constructor.name:', doc.constructor.name);
          console.log('[MONGOOSE DEBUG] doc.constructor.modelName:', doc.constructor.modelName);
          console.log('[MONGOOSE DEBUG] subdoc.constructor.name:', subdoc.constructor.name);
          console.log('[MONGOOSE DEBUG] subdoc.$basePath:', subdoc.$basePath);
          console.log('[MONGOOSE DEBUG] fullPathToSubdoc:', fullPathToSubdoc);
          console.log('[MONGOOSE DEBUG] subdocParent.constructor.name:', subdocParent.constructor.name);
          console.log('[MONGOOSE DEBUG] subdocParent keys:', Object.keys(subdocParent));
          console.log('[MONGOOSE DEBUG] doc._id:', doc._id);
          throw new Error(`subdocParent.$__ is null. Model: ${doc.constructor.modelName}, path: ${fullPathToSubdoc}, subdocParent type: ${subdocParent.constructor.name}`);
        }
        if (
          doc.$isModified(fullPathToSubdoc, null, modifiedPaths) &&
  ...
  for (const pathToCheck of paths) {
    if (doc.$__schema.nested[pathToCheck]) {
      ...
      // Re: gh-8468
      Object.keys(flat)
        .filter((path) => !doc.$__schema.singleNestedPaths.hasOwnProperty(path))
        .forEach(addToPaths);
    }
  }
  ...
  for (const path of paths) {
    ...
    if (
      _pathType.$parentSchemaDocArray &&
      typeof _pathType.$parentSchemaDocArray.path === 'string'
    ) {
...
Document.prototype.$__validate = function (pathsToValidate, options, callback) {
  ...
  if (validateAllPaths) {
    ...
    const pathDetails = _getPathsToValidate(
      this,
      pathsToValidate,
      pathsToSkip,
      options && options._nestedValidate,
    );
  ...
  if (paths.length === 0) {
    return immediate(function () {
      ...
      if (error) {
        return _this.$__schema.s.hooks.execPost(
          'validate:error',
          _this,
          [_this],
          { error: error },
  ...
  function validatePath(path) {
    ...
    immediate(function () {
      ...
      const scope =
        _this.$__.pathsToScopes != null && path in _this.$__.pathsToScopes
...
Document.prototype.$isValid = function (path) {
  if (
    this.$__.validationError == null ||
    Object.keys(this.$__.validationError.errors).length === 0
  ) {
    return true;
  }
  if (path == null) {
...
Document.prototype.$toObject = function (options, json) {
  ...
  const depopulate =
    options._calledWithOptions.depopulate ??
  ...
  const schemaFieldsOnly =
    options._calledWithOptions.schemaFieldsOnly ??
  ...
  const getters =
    options._calledWithOptions.getters ?? options.getters ?? defaultOptions.getters ?? false;
  ...
  const virtuals =
    options._calledWithOptions.virtuals ??
...
function applyVirtuals(self, json, options, toObjectOptions) {
  ...
  let v;
  const aliases =
    typeof (toObjectOptions && toObjectOptions.aliases) === 'boolean'
...
function applySchemaTypeTransforms(self, json) {
  ...
  for (const path of paths) {
    ...
    const topLevelTransformFunction =
      schematype.options.transform ?? schematype.constructor?.defaultOptions?.transform;
    const embeddedSchemaTypeTransformFunction =
...
function throwErrorIfPromise(path, transformedValue) {
  if (isPromise(transformedValue)) {
    throw new Error(
      '`transform` function must be synchronous, but the transform on path `' +
        path +
        '` returned a promise.',
    );
  }
...
Document.prototype.$restoreModifiedPathsSnapshot = function $restoreModifiedPathsSnapshot(
  snapshot,
) {
...
Document.prototype.$__hasOnlyPrimitiveValues = function $__hasOnlyPrimitiveValues() {
  return (
    ...
    (this._doc == null ||
      Object.values(this._doc).every((v) => {
        return (
          v == null ||
          typeof v !== 'object' ||
